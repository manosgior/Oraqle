...
class KnowledgeDistillationKLiNQ:
    def __init__(self, teacher_model, student_model, nn_config, config, distillation_setting):
        ...
        self.device_type = 'mps' #'nn_config.get('device', 'cpu')
    ...
    def validate(self, loader, model, criterion):
        ...
        with torch.no_grad():
            for inputs, labels in loader:
                ...
                # inputs = inputs[:, 1000:].view(inputs.size(0), 31)  # Shape: (batch_size, 2)
                remaining_length = (inputs.shape[1]) - 1000
                inputs = inputs[:, 1000:].view(inputs.size(0), remaining_length)
    ...
    def train_knowledge_distillation(self, best_teacher_model_path, X_train, y_train, X_val, y_val, trace_length):
        ...
        for epoch in range(self.num_epochs):
            ...
            for inputs, labels in train_loader:
                ...
                teacher_inputs = inputs[:, :1000].view(inputs.size(0), 1000)  # Shape: (batch_size, 1000)
                remaining_length = (inputs.shape[1]) - 1000
                student_inputs = inputs[:, 1000:].view(inputs.size(0), remaining_length)
            ...
            if best_val_loss is None or val_loss < best_val_loss - self.delta:
                ...
                counter = 0
                student_best_model_path = f"{self.model_save_directory}/{self.qubit_name}_trace_{self.trace_length}_best_model_kd_student_{self.distillation_setting}.pth"
    ...
    def test_best_student_model(self, X_test, y_test, best_student_model_path):
        ...
        weights = torch.load(best_student_model_path, map_location=device)
        self.student_model.load_state_dict(weights)
        self.student_model.to(device)  # Move model to device
        ...
        with torch.no_grad():
            ...
            for inputs, labels in test_loader:
                ...
                remaining_length = inputs.shape[1] - 1000
                inputs = inputs[:, 1000:].view(inputs.size(0), remaining_length).to(device)  # Move to device
                labels = labels.to(device).float().unsqueeze(1)  # Move labels to device

                outputs = self.student_model(inputs)  # Forward pass
                probabilities = torch.sigmoid(outputs)
                predicted = (probabilities > 0.5).float()
    ...
    def test_best_teacher_model(self, X_test, y_test, best_model_path, trace_length):
        test_loader = get_data_loader(X_test, y_test, batch_size=self.batch_size)
        ...
        with torch.no_grad():
            ...
            for inputs, labels in test_loader:
                ...
                if isinstance(self.teacher_model, KLiNQTeacherModel):
                    model_inputs = inputs[:, :1000].view(inputs.size(0), 1000)  # Shape: (batch_size, 1000)
                else:
                    pass

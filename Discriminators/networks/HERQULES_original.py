...
def get_train_val_and_test_set(trace, y, num_qubits=5, NUM_TRAIN_VAL = 3000, NUM_TEST = 7000, NUM_VAL_RATIO = 0.53, trace_length=TRACE_LENGTH):
    ...
    # 2. Adjust requested counts if they exceed what is available
    total_requested = NUM_TRAIN_VAL + NUM_TEST
    if total_requested > min_available:
        scale = min_available / total_requested
...
train_file_name = "/data/five_qubit_data/DRaw_C_Tr_v0-001"
test_file_name = "/data/five_qubit_data/DRaw_C_Te_v0-002"

SAMPLE_SEED = 42


MAX_TRAIN_SAMPLES = 192000
MAX_TEST_SAMPLES = 448000
...
demodulate_multiplexed_traces(iq_traces=all_data, qubit_frequencies=freq_readout, sampling_rate=sampling_rate)
...
def train(run_pre_filter=False,
    ...
    with T.profiler.profile(
        ...
        for epoch in range(0, max_epochs):
            ...
            if acc_val >= best_acc:
                best_acc = acc_val
                T.save(net.state_dict(), os.path.join(path, f'best_epoch_{TRACE_LENGTH}.pth'))

    print("Finished  training")
    print("Best acc on val dataset: {}".format(best_acc))

    net.load_state_dict(T.load(os.path.join(path, f'best_epoch_{TRACE_LENGTH}.pth')))
    acc_test, acc_test_per_qubit = accuracy(net, test_ldr)
    print("Acc on test dataset: {}".format(acc_test))
    print("Acc per qubit: {}".format(acc_test_per_qubit))
    #print("Acc per qubit: {}".format(acc_test_per_qubit))
...
def test():
    ...
    """
    model = Net_rmf()
    model.load_state_dict(T.load(os.path.join("./saved_models", f'best_epoch_{TRACE_LENGTH}.pth')))
